...
def _output_locus_tbl(result, te_idx, bam_path: str, prefix: str):
    """Write locus-level count table for TElocal."""
    fname = f"{prefix}.cntTable"
    try:
        f = open(fname, "w")
    except IOError:
    ...
    all_keys = sorted(set(result.gene_counts.keys()) | set(te_locus_counts.keys()))
    for key in all_keys:
        val = 0
        if key in result.gene_counts:
            val = int(result.gene_counts[key])
        elif key in te_locus_counts:
            val = int(te_locus_counts[key])
        f.write(f"{key}\t{val}\n")
...
def local(
    ...
    prefix: str = typer.Option(
        "TElocal_out", "--project", help="Name of this project."
    ),
    outdir: Optional[str] = typer.Option(
        None, "--outdir", help="Directory for output files. Default: current directory."
    ),
    ...
    """Count TE expression at locus level and gene expression from an RNA-seq BAM file."""
    if version:
        typer.echo("TElocal 0.1.0")
        raise typer.Exit()
    ...
    _output_locus_tbl(result, te_idx, bam, prefix)
    info(f"Output written to {prefix}.cntTable")
